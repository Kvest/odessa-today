...
public class MaskedEditText extends EditText implements TextWatcher {
    ...
    public MaskedEditText(Context context, AttributeSet attrs) {
        ...
        String placeholderValue = attributes.getString(R.styleable.MaskedEditText_placeholder_char);
        if (placeholderValue == null) {
            placeHolder = ' ';
        } else {
            placeHolder = placeholderValue.charAt(0);
        }
...
//					return false;
    ...
    private void cleanUp() {
        ...
        if(hasHint()) {
            ...
            this.setText(mask.replace(charRepresentation, placeHolder));
        }
        editingBefore = false;
    ...
    private String makeMaskedText() {
        char[] maskedText = mask.replace(charRepresentation, placeHolder).toCharArray();
        for(int i = 0; i < rawToMask.length; i++) {
            if(i < rawText.length()) {
                maskedText[rawToMask[i]] = rawText.charAt(i);
            }
            else {
                maskedText[rawToMask[i]] = placeHolder;
            }
        }
